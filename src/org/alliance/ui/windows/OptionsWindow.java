...
public class OptionsWindow extends XUIDialog {
	...
	private boolean apply() throws Exception {
		...
		ui.getCore().getNetworkManager().getUploadThrottle().setRate(
				settings.getInternal().getUploadthrottle() * KB);
		if (uploadThrottleBefore != settings.getInternal().getUploadthrottle())
			ui.getCore().getNetworkManager().getBandwidthOut()
					.resetHighestCPS();
		return true;
	}
	...
	public void EVENT_addrule(ActionEvent e) {
		try {
			new AddRuleWindow(ui);
			Routerule rule = ui.getCore().getSettings().getRulelist().get(ruleListModel.size());
			ruleListModel.add(ruleListModel.size(), rule);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		ruleList.revalidate();
	...
	public void EVENT_editrule(ActionEvent e) {
		if (ruleList.getSelectedIndex() != -1) {
			...
			try {
				new AddRuleWindow(ui, ruleIndex, temp.getHumanreadable());
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			ruleList.revalidate();
