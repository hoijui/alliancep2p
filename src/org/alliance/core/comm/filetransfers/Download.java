...
public class Download {
    ...
    public enum State { WAITING_TO_START, LOADING_FD, DOWNLOADING, COMPLETED };
    ...
    public void blockMaskReceived(int srcGuid, int hops, BlockMask bm) {
        if (hops > 0) {
            if(T.t)T.debug("Ignoring block mask coming from "+hops+" hops away.");
            return;
        }
        ...
        if(T.t)T.debug("Received blockmask from "+srcGuid+": "+bm+" size: "+bm.length());
        ...
        if (connections.size() == 0) {
            try {
                if(T.t)T.info("Got first block mask - starting download.");
                openDownloadConnection(srcGuid);
            } catch (IOException e) {
                if(T.t)T.warn("Could not connect to "+srcGuid+". Waiting for next block mask: "+e);
            }
        } else {
            if (getConnectionByGUID(srcGuid) == null) {
                try {
                    if (fd != null && getInterestingBlocks(bm).isEmpty()) {
                        if(T.t)T.debug("Remote has blocks of file, but no that I'm interested of: "+srcGuid);
                    } else {
                        if (connections.size() < manager.getMaxConnectionsPerDownload()) {
                            if(T.t)T.debug("Opening dl connection to another friend");
                            openDownloadConnection(srcGuid);
                        } else {
                            if(T.t)T.trace("Not opening more connection for download "+this+". have already: "+connections.size());
                        }
                    }
                } catch (IOException e) {
                    if(T.t)T.warn("Could not connect to "+srcGuid+". Waiting for next block mask: "+e);
    ...
    private Connection getConnectionByGUID(int srcGuid) {
        for(DownloadConnection c : connections.values()) {
            if (c.getRemoteUserGUID() == srcGuid) return c;
        }
        return null;
    }
    ...
    public void fileDescriptorReceived(DownloadConnection source, FileDescriptor fd) throws IOException {
        if (this.fd != null) {
            if(T.t)T.info("Already has FD. Ignore the new one and start download for this connection.");
            if(T.t)T.ass(source.readyToStartDownload(),"Not ready to start download?");
            if (needsMoreDownloadConnections()) source.startDownloadingBlock();
            return;
        }
        if(T.t)T.debug("Received file descriptor "+fd);

        this.fd = fd;
        setState(State.DOWNLOADING);

        ArrayList<DownloadConnection> al = new ArrayList<DownloadConnection>();
        for(DownloadConnection c : connections.values()) al.add(c);
    ...
    private boolean needsMoreDownloadConnections() {
        ...
        if (!b) if(T.t)T.trace("No need for more donwload connections.");
        return b;
    }
    ...
    public boolean isDownloadingFd() {
        for(DownloadConnection c : connections.values()) if (c.isDownloadingFd()) return true;
        return false;
    }
    ...
    public void connectionEstablished(DownloadConnection downloadConnection) {
        ...
        if(T.t)T.trace("Connection established: "+downloadConnection+" connections: "+connections.size());
    }

    public int selectBestBlockForDownload(Friend remoteFriend) throws IOException {
        if(T.t)T.debug("Selecting best block for download. Remote: "+remoteFriend);
        BlockMask bm = blockMasks.get(remoteFriend.getGuid());
        if (bm == null) {
            if(T.t)T.info("Ehh. Don't know anything about this friends block mask. Can't download.");
            return -1;
        }
        ...
        if (bf != null) highestBlockNumber = bf.getHighestCompleteBlock();
    ...
    public void abortAndRemovePerfmanently() throws IOException {
        for(DownloadConnection c : connections.values()) {
            if (c.isConnected()) c.sendGracefulClose();
        }
    ...
    public int getETAInMinutes() {
        if (fd == null || bandwidth.getAverageCps() == 0) return -1;
    ...
    public void signalBlockComplete(int blockNumber) throws IOException {
        ...
        if (signalBlockCompleteCounter > SEND_BLOCKMASK_UPDATE_INTERVAL_IN_BLOCKS) {
            ...
            if (list != null) for(Friend f : list) {
                FriendConnection fc = f.getFriendConnection();
                if (fc != null) {
                    if(T.t)T.info("Friend "+f+" is interested in file "+root+". Sending updated block mask to him.");
    ...
    public static Download createFrom(ObjectInputStream in, DownloadManager m) throws IOException {
        try {
            ...
            if(T.t)T.ass(r == h.array().length,"Incorrect length when deserializing download");
