...
//        dett borde räcka med en signalInterestToSend här istället
...
//        Packet.printBuf(buf);
        ...
        while (receivePacket.getPos() >= 2) {
            ...
            if (receivePacket.getPos() >= len) {
                ...
                if (c == null) {
                    if(T.t)T.debug("Connection got closed. Probably because of duplicate connection.");
                    receivePacket = null;
                    return;
                }
                if(T.t)T.ass(c == this,"We're about to send a packet to a swapped connection. The right way to go is to send the raw data to the swapped connection using received(...).");
                packetsReceived++;
