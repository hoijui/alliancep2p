...
public class UPnPManager extends ControlPoint implements DeviceChangeListener {
    ...
    public boolean start() {
        if(T.t)T.info("Starting UPnP Manager.");
    ...
    public InetAddress getNATAddress() throws UnknownHostException {
        ...
        if(getIP == null) {
            if(T.t)T.debug("Couldn't find GetExternalIPAddress action!");
            return null;
        }


        if (!getIP.postControlAction()) {
            if(T.t)T.debug("couldn't get our external address");
            return null;
        }
    ...
    public void deviceAdded(Device dev) {
        synchronized(DEVICE_LOCK) {
            if(T.t)T.trace("Device added: " + dev.getFriendlyName());

            // we've found what we need
            if (_service != null && _router != null) {
                if(T.t)T.info("we already have a router");
                return;
            }
            ...
            if (_router == null) {
                if(T.t)T.info("didn't get router device");
                return;
            }
            ...
            if (_service == null) {
                if(T.t)T.debug("couldn't find service");
                _router=null;
            } else {
                if(T.t)T.info("Found service, router: " + _router.getFriendlyName() + ", service: " + _service);
                DEVICE_LOCK.notify();
                stop();
            }
        }
    ...
    private void discoverService() {
        ...
        for (Iterator iter = _router.getDeviceList().iterator();iter.hasNext();) {
            ...
            if(T.t)T.debug("found "+current.getDeviceType()+", size: "+l.size() + ", on: " + current.getFriendlyName());
            ...
            for (int i=0;i<current.getDeviceList().size();i++) {
                ...
                if(T.t)T.debug("found "+current2.getDeviceType() + ", on: " + current2.getFriendlyName());
    ...
    public int mapPort(int port) throws IOException {
        if(T.t)T.trace("Attempting to map port: " + port);
...
*/
        ...
        if (!addMapping(tcp)) {
            if(T.t)T.debug(" couldn't map tcp port. cleaning up...");
            port = 0;
            tcp = null;
        }
    ...
    private boolean addMapping(Mapping m) {
        ...
        if(add == null) {
            if(T.t)T.debug("Couldn't find AddPortMapping action!");
            return false;
        }
        ...
        if(T.t)T.trace("Post succeeded: " + success);
        return success;
    }
    ...
    private boolean removeMapping(Mapping m) {
        ...
        if(remove == null) {
            if(T.t)T.debug("Couldn't find DeletePortMapping action!");
            return false;
        }
        ...
        if(T.t)T.debug("Remove succeeded: " + success);
        return success;
    }
    ...
    public void shutdown() {
        if (mappedPort != 0 ) {
            ...
            Thread cleaner = new Thread("UPnP Cleaner") {
                public void run() {
                    if(T.t)T.debug("start cleaning");
                    removeMapping(tcp);
                    if(T.t)T.debug("done cleaning");
                }
            };
            ...
            try {
                if(T.t)T.debug("waiting for UPnP cleaners to finish");
                cleaner.join(30000); // wait at most 30 seconds.
            } catch(InterruptedException ignored){}
            if(T.t)T.debug("UPnP cleaners done");
        }
    }
    ...
    public void init() throws IOException {
        ...
        if (natted) {
            ...
            if (mappedPort != core.getNetworkManager().getServerPort()) {
                if(T.t)T.error("Mapped incorrect port in router: "+mappedPort);
            } else {
    ...
    private class StaleCleaner implements Runnable {
        ...
        public void run() {
            ...
            if(T.t)T.debug("Looking for stale mappings...");
            ...
            if(getGeneric == null) {
                if(T.t)T.debug("Couldn't find GetGenericPortMappingEntry action!");
                return;
            }
            ...
            try {
                for (int i=0;;i++) {
                    ...
                    if(T.t)T.debug("Stale Iteration: " + i + ", generic.input: " + list(getGeneric.getInputArgumentList()) + ", generic.output: " + list(getGeneric.getOutputArgumentList()));
                ...
                if(T.t)T.error("NFE reading mappings!"+bad);
                //router broke.. can't do anything.
                return;
            }

            if(T.t)T.debug("Stale cleaner found "+mappings.size()+ " total mappings");

            // iterate and clean up
            for (Iterator iter = mappings.iterator();iter.hasNext();) {
                Mapping current = (Mapping)iter.next();
                if(T.t)T.debug("Analyzing: " + current);
                ...
                if (current._description.equals(TCP_PREFIX+getGUIDSuffix())) {
                    ...
                    // remove it.
                    if(T.t)T.debug("mapping "+current+" appears to be stale");
