...
public class FriendConnector extends Thread {
            ...
            for(Friend f : al) {
                ...
                while(actionQue.size() > 0) {
                    ...
                    if (a.sleepDelay != null && a.sleepDelay > 0) {
                        if(T.t)T.trace("Sleeping "+a.sleepDelay+"ms...");
                        try { Thread.sleep(a.sleepDelay);} catch (InterruptedException e) {}
                    }
                    connect(a.f);
                }
    ...
    public void queHighPriorityConnectTo(Friend f, int dealyInMs) {
        if (f.isConnected()) {
            if(T.t)T.info("Already connected to "+f+", not connecting again.");
            return;
        }
        if(T.t)T.info("Queuing action: connect to "+f+" in "+dealyInMs+" ms.");
